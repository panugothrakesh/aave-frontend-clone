...
function Bars() {
    ...
    // Update the container width dynamically and calculate the scale factor
    useEffect(() => {
        const updateContainerWidth = () => {
            if (maskRef.current) {
                const width = maskRef.current.offsetWidth;
                setContainerWidth(width);
                // Set the scale factor based on the current width relative to the max width (986px)
                setScaleFactor(Math.min(1, width / 986)); // Ensure scale never exceeds 1 (full size)
            }
        };

        updateContainerWidth(); // Initial width
        window.addEventListener('resize', updateContainerWidth); // Update width on resize
    ...
    return (
        <div
            className="block animate-see overflow-visible -mt-[110px] mb-0 mx-auto px-12 w-full max-w-[1082px]"
            ref={maskRef}
